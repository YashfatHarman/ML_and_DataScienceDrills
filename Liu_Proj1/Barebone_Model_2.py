...
# This one needs to be locally conencted with no shared weight.

model2 = Sequential()

model2.add(LocallyConnected2D(64, (3, 3), input_shape = input_shape, activation = "tanh", kernel_initializer = "he_normal"))
model2.add(LocallyConnected2D(64, (3, 3), activation = "relu", kernel_initializer = "he_normal"))
model2.add(LocallyConnected2D(64, (3, 3), activation = "relu", kernel_initializer = "he_normal"))
...
model2.compile(loss = keras.losses.categorical_crossentropy, optimizer = keras.optimizers.RMSprop(), 
              metrics = ["accuracy"])
...
h = model2.fit(X_train_train, y_train_train, batch_size = batch_size, epochs = epochs, validation_data = (X_train_val, y_train_val), verbose = 1)

model2.save_weights("model2_weights.h5")
pickle.dump(h.history, open("model2_history.pk","wb"))
